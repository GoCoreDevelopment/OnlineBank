...
```bash
# unit-тесты (хэширование паролей, JWT, bind, бизнес-логика переводов)
go test ./...

# интеграционные тесты репозитория — требуют запущенного PostgreSQL
go test -tags=integration ./internal/repository/
```

Unit-тесты не требуют внешних зависимостей и проверяют логику в изоляции (репозиторий
подменяется моками). Тесты, которым нужна реальная БД, вынесены под build-tag `integration`.
